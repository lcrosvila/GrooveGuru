...
for audio_file in song_list:
    ...
    for c in chart[1:]:
        ...
        if file_name not in os.listdir(save_dir):
            ...
            indices = [
                i for i, x in enumerate(notes) if x == "," or x == "" or x == ";"
            ]
            ...
            zero_additions = [
                16 // (indices[i + 1] - indices[i] - 1) for i in range(len(indices) - 1)
            ]
            ...
            indices_new = [
                i
                for i, x in enumerate(new_notes[:-1])
                if x == "," or x == "" or x == ";"
            ]
            lengths_new = [
                indices_new[i + 1] - indices_new[i] - 1
                for i in range(len(indices_new) - 1)
            ]
