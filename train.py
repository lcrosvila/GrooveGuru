...
class TrainingWrapper(pl.LightningModule):
    ...
    def _log_model_visualizations(self, batch, batch_idx):
        ...
        wandb.log({"difficulty_self_similarity": wandb.Image(self_similarity)}, step=self.global_step)
        ...
        wandb.log({"time_self_similarity": wandb.Image(self_similarity)}, step=self.global_step)

    def _log_predicted_and_true_activity_maps(self, batch, batch_idx):
        predicted_activity_map = self.model(batch['mel_spectrogram'], batch['difficulty'])
        ...
        fig = plt.gcf()  # your current figure
