...
sys.path.insert(
    0, "/opt/homebrew/lib/python3.10/site-packages"
)  # to make sure I get all the pip libraries
...
def preprocess_audio(audio_path, offset):
    ...
    audio_embed = clap_model.get_audio_embedding_from_data(x=[audio], use_tensor=False)
...
class DDRDataLoader(torch.utils.data.Dataset):
    def __init__(self, data_files):
        ...
        for file in data_files:
            ...
            audio_array, input_data = preprocess_audio(
                data["audio_file"].item(), offset
            )
            # remove the batch dimension from input_data
            input_data = input_data.squeeze(0)
            target_aux = [self.token_to_index[token] for token in data["notes"]]
...
class TransformerDecoder(nn.Module):
    def __init__(self, vocab_size, output_size, hidden_size, num_layers, num_heads):
        ...
        self.transformer = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(
                hidden_size, num_heads, dim_feedforward=hidden_size
            ),
            num_layers,
        )
    ...
    def generate(self, audio_emb, level, max_len=100, temperature=1.0):
        ...
        start_token = torch.tensor(
            [token_to_index[""]], dtype=torch.long, device=device
        )
        level_token = torch.tensor(
            [token_to_index[level]], dtype=torch.long, device=device
        )
        x = torch.cat((start_token, level_token), dim=0).unsqueeze(0)  # shape (1, 2)
        ...
        with torch.no_grad():
            for _ in range(max_len):
                ...
                b, t = x.size()
                pos = torch.arange(t, t + 1, dtype=torch.long, device=device).unsqueeze(
                    0
                )
                tok_emb = self.emb(x)  # Token embeddings of shape (b, t, n_embd)
                pos_emb = self.pos(pos)  # Position embeddings of shape (1, t, n_embd)
                ...
                logits = self.fc1(x_output)  # Logits of shape (b, t, vocab_size)
...
def get_model(vocab_size, output_size, hidden_size, num_layers, num_heads):
    model = TransformerDecoder(
        vocab_size, output_size, hidden_size, num_layers, num_heads
    )
    return model
        ...
        for i, (audio_array, audio_emb, SM_x, SM_y) in enumerate(val_dataloader):
            ...
            if i == batch_idx:
                ...
                generated_tokens = model.generate(
                    audio_emb[0], level=actual_chart[0], max_len=max_output_size
                )
                generated_chart = "\n".join(
                    [index_to_token[i.item()] for i in generated_tokens[0]]
                )

                log_audio_chart_table(
                    audio_array,
                    generated_chart,
                    actual_chart,
...
def log_audio_chart_table(audio, predicted, labels, probs):
    ...
    for i in range(len(audio)):
        table.add_data(
            wandb.Audio(audio[i], sample_rate=sr), predicted[i], labels[i], probs[i]
        )
